...
同步用 `BookId`、`bookmarkId`、`reviewId` 和统计日期去重。重复键会跳过并写入运行摘要，不自动删除。划线和想法保留正文；书签 API 只有数量，没有可导出的书签内容。书籍同步会从书架填入作者、分类、封面、阅读时长和阅读进度，并补齐“年”关联和缺失的灰色书本图标，确保书籍出现在既有书架视图。阅读时长使用 `/book/getprogress` 的秒数，进度从微信读书的 0–100 换算为 Notion 的 0–1 百分比。`metadata_backfill` 不导入笔记、划线，但会从微信读书的逐月日统计回填日/周/月/年阅读时长；适合修复旧记录。增量模式仅更新当月统计并以笔记本 `sort` 跳过未变化的明细；`full` 可重新扫描全部明细。运行失败后按 ID 安全重试，先看 Actions 的失败日志和摘要。
